...
import { Play, Pause, Upload, TrendingUp, TrendingDown, Minus, Calendar, Activity, Grid, Gauge, Star, LayoutGrid, Award, Trophy, Crown, ArrowUpRight, ArrowDownRight, Target, Zap, BarChart2, Filter, Clock, Link, FileText, X, ChevronDown, RefreshCw, Globe } from 'lucide-react';
...
const AnalyticsView = ({ data, t }: { data: ParsedData, t: any }) => {
  ...
  return (
    <div className="flex-1 w-full min-w-full p-6 bg-slate-50 h-full overflow-y-auto custom-scrollbar font-mju flex flex-col">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6 pb-10 w-full">
        {analysisResults.stats.map((stat, idx) => (
...
export default function App() {
  ...
  return (
    <div className="absolute inset-0 flex flex-col w-full bg-slate-100 text-slate-900 overflow-hidden font-mju">
      ...
      {/* Deepened pt-10 md:pt-12 to completely dodge the Gemini/Chrome iframe top-edge overlay interception */}
      <header className="bg-white border-b border-slate-200 px-4 md:px-6 pt-10 md:pt-12 pb-4 flex items-center shadow-sm z-[9999] relative w-full min-h-[90px] shrink-0 gap-2 md:gap-4">
        
        {/* Left: Logo Block */}
        <div className="flex-1 flex justify-start items-center gap-3 relative z-10">
           <img src="https://upload.cc/i1/2026/01/29/BauOeN.png" alt="CIS Logo" className="h-7 md:h-8 w-auto object-contain" />
           <div>
             <h1 className="text-base md:text-lg font-bold text-slate-800 leading-tight">{t.title} <span className="text-blue-600">V4.52</span></h1>
             <p className="text-[10px] md:text-xs text-slate-500">{t.desc}</p>
           </div>
        </div>

        {/* Center: Function Tabs (Flex-1 allocation creates perfect centering) */}
        <div className="hidden lg:flex justify-center items-center shrink-0 relative z-20">
           <div className="bg-slate-100 p-1.5 rounded-lg flex items-center gap-1 border border-slate-200 shadow-sm">
             <button 
               type="button"
      ...
      <div className="lg:hidden w-full bg-white border-b border-slate-200 px-4 py-2 overflow-x-auto hide-scrollbar z-[9998] relative shadow-sm shrink-0">
         <div className="bg-slate-100 p-1 rounded-lg flex items-center gap-1 border border-slate-200 w-max mx-auto">
           <button 
             type="button"
             onClick={() => setViewMode('curve')}
             className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-all cursor-pointer select-none ${viewMode === 'curve' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200'}`}
           >
             <TrendingUp size={16} /> {t.curve}
           </button>
           <button 
             type="button"
             onClick={() => setViewMode('grid')}
      ...
      {/* Main Stage */}
      <div className="flex flex-1 overflow-hidden w-full relative bg-slate-50 z-0">
